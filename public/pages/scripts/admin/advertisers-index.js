...
function func_Delete_Advertiser(advertiserid_local)
{
    ...
    content='' +
    '<form name="frm_delete_advertiser" id="frm_delete_advertiser" method="post">' +
    '   <input type="hidden" name="_method" value="DELETE">'+
    '   <input type="hidden" name="hdn_advertiser_id" value="'+advertiserid_local+'">' +
    '   <input type="hidden" name="_token" value="'+csrf_token+'">'+
    '   <h4 style="font-weight:bold;">Please enter delete in capital letters to delete this advertiser.</h4>'+
                ...
                callback: function() {
                    ...
                    else
                    {
                        if(title=='DELETE')
                        {
                            ...
                            jQuery('#frm_delete_advertiser').submit();
                            /*
                            jQuery.ajax({
                                type: "POST",
                                url: url+"/delete-customer-detail.php",
                                dataType:'json',
                                data: 'action=delete_customer_detail&userid='+uid+'&customerid='+advertiserid_local,
...
jQuery(document).ready(function(){
    ...
    jQuery('#anc_create_csv').on('click',function(){
        //var selected_id="x9pOBT2XqS,9DRSj83gK4";
        var selected_id = '';
        var set = $('#datatable_ajax > tbody > tr > td:nth-child(1) input[type="checkbox"]');
        var tot_rec = 0;
        $(set).each(function() {
        ...
        $('#selected_totrec').html(tot_rec+' Records Found.');
        $('#selected_id').val(selected_id);
        jQuery("#modal_create_csv").modal('show');

    });

    jQuery('#anc_create_pdf').on('click',function(){
        //var selected_id="x9pOBT2XqS,9DRSj83gK4";
        var selected_id = '';
        var set = $('#datatable_ajax > tbody > tr > td:nth-child(1) input[type="checkbox"]');
        var tot_rec = 0;
        ...
        $('#selected_totrec_pdf').html(tot_rec+' Records Found.');
        $('#selected_id_pdf').val(selected_id);
        jQuery("#modal_create_pdf").modal('show');
    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "datatable/show1", // ajax source
                    "method":"post",
