...
jQuery(document).ready(function(){
	...
	/*
    $('#datatable_ajax').on('click', 'input[type="checkbox"]', function() {

        var url_str="";

		var publisher_id=jQuery(this).val();
		url_str+='publisher_id='+publisher_id;

		jQuery('.chk_'+publisher_id).each(function(){

			var field_name=jQuery(this).data('field-name');
		    console.log(jQuery(this).data('field-name'));

			if(jQuery(this).is(':checked'))
			{
				url_str+='&'+field_name+'=1';
			}
			else
			{
				url_str+='&'+field_name+'=0';
			}
	    ...
	    jQuery.ajax({
			type: "POST",
			url: publisher_on_boarding_submit_url,
			dataType:'json',
			data: url_str,
    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": data_table_list_ajax_url, // ajax source
                    "method":"post",
                     headers: { 'X-CSRF-TOKEN' : csrf_token },
                },
                "order": [
		        ...
		        "drawCallback": function( settings ) {
			        var api = this.api();
		            api.$('td .chk_publisher_on_boarding').each( function () {
		                if(jQuery(this).is(':checked'))
		                {
		                    jQuery(this).closest('td').removeClass('danger');
		                    jQuery(this).closest('td').addClass('success');
		                }
		                else
		                {
		                    jQuery(this).closest('td').removeClass('success');
		                    jQuery(this).closest('td').addClass('danger');
		                }
		            } );
			    }
            }
        ...
        console.log(grid_ele);
         ...
         grid_ele.on('click', 'input[type="checkbox"]', function() {
